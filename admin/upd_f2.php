...
<body>
	...
	<div class="container">
		<div class="span12">
			<form method="POST" action="upd_f2.php">
		...
		<form method="POST" action="upd_f2.php">
			<fieldset>
				<legend class="text-success col-lg-12">
					Update Fees for The class <?php echo "2"?>
				</legend>
				<div class="table table-bordered table-success" style="padding-top: 20px">
					...
					<div class="row">
						...
						<div class="form-group">
							...
							<input type="number" name="class" class="form-control input-md" value="<?php echo '2'?>" readonly>
						</div>
					</div>
					<div class="col-lg-4">
						<div class="form-group">
	...
	<div class="container">
		...
		<div class="row">
			<?php
			$disp="SELECT votehead,amount FROM fees WHERE class='2'";
	...
	<div class="container">
		<table style="float: right">
			<?php 
						$sql1="SELECT * FROM fees WHERE class='2'";
	...
	if(isset($_POST['submit'])){
		...
		$upd="UPDATE fees SET amount='$amount' WHERE votehead='$votehead' AND class='2'";
		$updq=mysqli_query($conn,$upd);
		if($upd){
			echo "<script>alert('Fees  Successfully Updated')</script>";
			echo "<script>window.open('upd_f2.php','_self');</script>";	
		}
	}
	?>
	<?php 
		$sql1="SELECT * FROM student WHERE class='2'";
		$retval1 = mysqli_query($conn,$sql1);
		while($rows1=mysqli_fetch_assoc($retval1)){
		$bal=$rows1['balance'];
		}
	if(isset($_POST['set_fees'])){
		$bal=$bal+$qty;
		$sum="UPDATE student SET fees_amount='$qty' WHERE class='2'";
		$bal1="UPDATE student SET balance='$bal' WHERE class='2'";
